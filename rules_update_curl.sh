...
curl -f -L -o "$TEMP_DIR/geoip.dat" "$GEOIP_URL" && echo "geoip.dat успешно загружен." || echo "Ошибка при загрузке geoip.dat."

echo "Загрузка geosite.dat..."
curl -f -L -o "$TEMP_DIR/geosite.dat" "$GEOSITE_URL" && echo "geosite.dat успешно загружен." || echo "Ошибка при загрузке geosite.dat."
